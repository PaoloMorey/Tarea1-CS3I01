...
def AES128Decryption(key: str, ct: str, padding: bool=True):
	...
	for i in range(len(tempArr)):
		block = tempArr[i]
		block=AESDecFirstRound(keys[10], block)
...
def AESDecRound(key: str, block: list):
	tempMat = XOR(block, key)	
	tempMat = invMixCols(tempMat)
	tempMat = invShiftRows(tempMat)	
	return invSubBytes(tempMat)
...
def shiftRows(arr: list):
	...
	for i in range(4):
		tempArr[i] = arr[i]
		tempArr[4+i] = arr[(i+1)%4 + 4]
		tempArr[8+i] = arr[(i+2)%4 + 8]
		tempArr[12+i] = arr[(i+3)%4 + 12]
	return tempArr
...
def invShiftRows(arr: list):
	...
	for i in range(4):
		tempArr[i] = arr[i]
		tempArr[4+i] = arr[(i+3)%4 + 4]
		tempArr[8+i] = arr[(i+2)%4 + 8]
		tempArr[12+i] = arr[(i+1)%4 + 12]
	return tempArr
